...
export async function readSuggestionSheets(spreadsheetId: string, range: string): Promise<string[]> {
    ...
    if (rows) {
      rows.forEach((row) => {
        ...
        if (status === 'Accept') {
          ...
          let issue: string = row[1]
          issue = issue.substring(0, 32)
    
          const unixTime: number = new Date(timestamp).getTime() / 1000
    
          const feedbackString: string = `new_feedback{app="suggestion",since="${timestamp}",issue="${issue}"} ${unixTime}`
