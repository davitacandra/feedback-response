...
export async function readNusaworkSheets(spreadsheetId: string, range: string): Promise<string[]> {
    ...
    if (rows) {
      rows.forEach((row) => {
        ...
        if (status === 'Accepted') {
          ...
          const feedbackString: string = `new_feedback{app="nusawork",since="${timestamp}",from="${fromUser}",issue="${issue}"} ${unixTime}`
